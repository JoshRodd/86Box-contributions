...
static void
terminal_finish_presentation(int result)
{
    if (result < 0) {
        /* Tolerate frames the pure-glass presenter cannot express (e.g. an
           incomplete scroll) so the guest keeps executing; the host display
...
void
terminal_video_present_text(void)
{
    terminal_video_frame *storage = &terminal_frames[monitor_index_global];
    if (!terminal_presenter || !storage->columns || !storage->rows)
        return;
    ...
    if (terminal_cursor_query == TERMINAL_CURSOR_WAITING)
        return;
    for (uint16_t row = 0; row < storage->rows; row++) {
        for (uint16_t column = 0; column < storage->columns; column++) {
            const size_t index = (size_t) row * TERMINAL_TEXT_COLUMNS + column;
            if (storage->cell_frame[index] != storage->frame)
                storage->cells[index] = (tigt_text_cell) { ' ', 0, 0, 0 };
        }
    }
    uint16_t cursor_column = 0, cursor_row = 0;
    int cursor_found = 0;
    for (uint16_t row = 0; row < storage->rows && !cursor_found; row++) {
        for (uint16_t column = 0; column < storage->columns; column++) {
            const tigt_text_cell *cell =
                &storage->cells[(size_t) row * TERMINAL_TEXT_COLUMNS + column];
            if (cell->flags & TIGT_TEXT_CURSOR) {
    ...
    const tigt_presenter_frame output = {
        storage->cells, storage->columns, storage->rows, TERMINAL_TEXT_COLUMNS,
        cursor_column, cursor_row, 60, 0
    };
    terminal_finish_presentation(tigt_presenter_present(terminal_presenter, &output));
}
                         ...
                         uint8_t mode, int monochrome, const uint8_t *pcjr_array)
{
    ...
    terminal_finish_presentation(result);
}
